...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        "partial",
        "Local memory files, redacted memory list/show/search, approval-gated curated memory writes/deletes, redacted session search, and SKILL.md discovery are available.",
        "primary",
        "Add stronger skill trust metadata and richer memory provenance controls.",
        ("aegis memory list", "aegis memory show <entry-id>", "aegis memory delete <entry-id> --approved", "/memory list"),
    ),
    Capability(
...
def capability_map(paths: RuntimePaths) -> dict[str, Any]:
    ...
    dynamic_commands = {
        ...
        "memory_sessions_skills": (f"{command} memory list", f"{command} memory show <entry-id>", f"{command} memory delete <entry-id> --approved", "/memory list"),

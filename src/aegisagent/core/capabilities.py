...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        "partial",
        "Planner, researcher, implementer, and reviewer workers can run through the active model provider route with local fallback, isolated sessions, bounded concurrency, visible role contracts, scoped usage ledger entries, durable searchable role artifacts, read-only artifact list/show/search, input-artifact handoff metadata, and audit receipts.",
        "primary",
        "Add approval-gated cross-delegation artifact reuse, richer role-specific tool budgets, higher-depth delegation controls, and final synthesis over artifact graphs.",
    ...
    Capability(
        ...
        "partial",
        "Local provider is default; ready OpenAI-compatible API-key routes can serve terminal chat and subagent workers, failed attempted external calls fall back to the local provider, and top-level plus worker usage is recorded in a terminal-visible ledger.",
        "primary",
    ...
    Capability(
        ...
        "partial",
        "Slack, Teams, webhook, MCP, browser, and Open WebUI readiness metadata is visible and gated; redacted connector packets can be drafted or approval-recorded into an outbox without network delivery; terminal web fetch and browser session records are available as approved actions.",
        "primary",
        "Wire real Slack, Teams, webhook, Open WebUI, and MCP adapters behind payload-bound approvals.",
        ("aegis connectors", "aegis connectors outbox", "aegis connectors draft slack --target <target> --message <body>", "/connectors send <name> | <target> | <message> | approve"),
    ),
    Capability(
        "gateway_web",
        "Gateway and Web GUI",
...
def capability_map(paths: RuntimePaths) -> dict[str, Any]:
    ...
    dynamic_commands = {
        ...
        "agents_subagents": (f"{command} agents", f"{command} agents artifacts", f"{command} agents delegate <task>", "/agents bg <task>"),
        ...
        "connectors_messaging_mcp_browser": (f"{command} connectors", f"{command} connectors outbox", f"{command} connectors draft slack --target <target> --message <body>", "/connectors send <name> | <target> | <message> | approve"),

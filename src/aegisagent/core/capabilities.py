...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        "partial",
        "Planner, researcher, implementer, and reviewer workers can run through the active model provider route with local fallback, isolated sessions, per-delegation concurrency caps, opt-in depth-2 reviewer nesting, visible role contracts, structured role-specific tool budget policies, scoped usage ledger entries, durable searchable role artifacts, read-only artifact list/show/search, background job monitor/status views, stage-to-stage input-artifact handoff metadata, approval-gated reuse of selected prior artifacts as bounded summary context for later foreground or detached delegations, coordinator final synthesis over artifact graphs, and audit receipts.",
        "primary",
        "Clarify bounded depth-2 topology controls and add richer execution backends without arbitrary recursive spawning.",
        ("aegis agents", "aegis agents monitor <job-id>", "aegis agents status <root-id>", "/agents bg <task> | depth 2"),
    ),
    Capability(
    ...
    Capability(
        ...
        "partial",
        "Webhook delivery is live only for the configured webhook route after explicit approval; Slack, Teams, Open WebUI, MCP, and browser connector metadata remain visible, gated, and outbox/readiness-only. Terminal web fetch and browser session records remain separate approved actions.",
        "primary",
        "Keep Slack, Teams, Open WebUI, and MCP delivery disabled until each adapter has its own payload-bound approval path, tests, and receipts.",
        ("aegis connectors configure webhook --url-env AEGIS_WEBHOOK_URL --enable", "aegis connectors send webhook --target <label> --message <body> --approved", "/connectors send webhook | <label> | <message> | approve"),
    ),
    Capability(
        "gateway_web",
        "Gateway and Web GUI",
...
def capability_map(paths: RuntimePaths) -> dict[str, Any]:
    ...
    dynamic_commands = {
        ...
        "agents_subagents": (f"{command} agents", f"{command} agents monitor <job-id>", f"{command} agents status <root-id>", "/agents bg <task> | depth 2"),

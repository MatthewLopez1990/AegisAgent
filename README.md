...
- Terminal install and update: the fresh-machine macOS/Linux installer is `/bin/sh -c "$(curl -fsSL https://raw.githubusercontent.com/MatthewLopez1990/AegisAgent/main/scripts/install.sh)"`. From a checkout, `aegisagent install`, `aegisagent install shim --approved`, `aegisagent update`, `aegisagent update --approved`, `/install`, and `/update` provide terminal lifecycle commands. The install shim writes `~/.local/bin/aegis` by default, and update runs `git pull --ff-only origin main` only after explicit approval.

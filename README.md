...
- `aegis skills` and `/skills` are discovery-only. They read local `SKILL.md` files as text, redact derived metadata, hash scanned content, optionally verify an `aegis-skill-trust.json` SHA-256 bundle manifest when present, and report `trusted`, `review`, or `quarantined` posture. Missing manifests are allowed metadata; invalid or mismatched manifests are quarantined. Signature fields are reported as provenance metadata until trusted-key verification exists. Discovery does not execute skills, install packages, invoke models, open browsers, or send network requests.

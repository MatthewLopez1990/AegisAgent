...
class CliTests(unittest.TestCase):
    ...
    def test_model_help_and_connect_shortcut_are_onboarding_friendly(self):
        with tempfile.TemporaryDirectory() as tmp:
            help_result = run_cli("model", "--help", cwd=tmp)
            self.assertEqual(help_result.returncode, 0, help_result.stderr)
            self.assertIn("examples:", help_result.stdout)
            self.assertIn("aegis model connect local", help_result.stdout)
            self.assertIn("aegis model connect openai", help_result.stdout)

            connected = run_cli("--json", "connect", "openai", cwd=tmp, extra_env={"OPENAI_API_KEY": "test-key"})
            self.assertEqual(connected.returncode, 0, connected.stderr)
            payload = json.loads(connected.stdout)
            self.assertEqual(payload["provider"], "openai/gpt-5.5")
            self.assertEqual(payload["env_handle"], "OPENAI_API_KEY")
            self.assertTrue(payload["env_present"])
    ...
    def test_mac_linux_installer_scripts_are_terminal_first(self):
        ...
        self.assertIn("git check-ref-format --branch", install_text)
        ...
        self.assertIn("$COMMAND_NAME connect local", install_text)
        self.assertIn("export OPENAI_API_KEY", install_text)
        self.assertIn("$COMMAND_NAME connect openai", install_text)
        self.assertIn("$COMMAND_NAME model doctor", install_text)
        ...
        self.assertIn("git check-ref-format --branch", update_text)
    ...
    def test_readme_leads_with_installed_user_terminal_flow(self):
        ...
        self.assertIn("curl --version", readme)
        self.assertIn("command -v aegis", readme)
        self.assertIn("aegis activation", readme)
        self.assertIn('AEGIS_PYTHON="/path/to/python3.12"', readme)
        connect_section = readme[connect_index:start_index]
        self.assertIn("aegis connect openai", connect_section)
        self.assertIn("aegis connect local", connect_section)
        ...
        self.assertIn("aegis connect openrouter --base-url https://openrouter.ai/api/v1 --model openai/gpt-4o-mini", connect_section)
    ...
    def test_readme_installed_user_commands_smoke_without_browser_launch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("aegis connect local", install.stdout)
            self.assertIn("aegis connect openai", install.stdout)
            self.assertIn("aegis model doctor", install.stdout)
            ...
            commands = [
                ...
                ("connect local", ["aegis", "connect", "local"]),
                ("connect openai", ["aegis", "connect", "openai"]),
            ...
            self.assertIn("AEGIS MODEL CONNECT", outputs["connect local"])
            self.assertIn("OPENAI_API_KEY", outputs["connect openai"])
            ...
            self.assertIn("AEGIS MODEL DOCTOR", outputs["model doctor"])
    ...
    def test_setup_quickstart_uses_installed_command_name(self):
        ...
        self.assertIn("aegis-test connect local", result.stdout)
    ...
    def test_install_status_and_shim_are_terminal_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('AEGIS_PYTHON="${AEGIS_PYTHON:-', script)
    ...
    def test_installed_shim_runs_activation_with_custom_command_name(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bin_dir = Path(tmp) / "bin"
            installed = run_cli("install", "shim", "--bin-dir", str(bin_dir), "--name", "aegis-test", "--approved", cwd=str(workspace), extra_env={"AEGIS_PYTHON": sys.executable})
            self.assertEqual(installed.returncode, 0, installed.stderr)

            shim = bin_dir / "aegis-test"
            script = shim.read_text(encoding="utf-8")
            self.assertIn(f"AEGIS_PYTHON_DEFAULT='{sys.executable}'", script)
            run_env = {**os.environ, "PATH": os.environ.get("PATH", "")}
            run_env.pop("AEGIS_PYTHON", None)
    ...
    def test_setup_health_audit(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            audit_path = Path(tmp) / ".aegisagent" / "audit.jsonl"
            audit_text = audit_path.read_text(encoding="utf-8")
            audit_path.write_text(audit_text.replace('"event_type"', '"tampered"', 1), encoding="utf-8")
            broken_health = run_cli("health", cwd=tmp)
            self.assertEqual(broken_health.returncode, 1)
            broken_payload = json.loads(broken_health.stdout)
            self.assertFalse(broken_payload["ok"])
            self.assertFalse(broken_payload["audit_chain_ok"])
    ...
    def test_model_provider_config_is_metadata_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            blocked_query_url = run_cli("model", "connect", "openai", "--base-url", "https://example.com/v1?token=secret#frag", cwd=tmp)
            ...
            self.assertIn("AEGIS MODEL DOCTOR", doctor.stdout)
            self.assertIn("checks", doctor.stdout)
            doctor_json = run_cli("--json", "model", "doctor", cwd=tmp)
            self.assertEqual(doctor_json.returncode, 0, doctor_json.stderr)
            checks = json.loads(doctor_json.stdout)["checks"]
    ...
    def test_models_alias_and_auth_readonly_forms_are_metadata_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            model_doctor = run_cli("models", "doctor", cwd=tmp)
            self.assertEqual(model_doctor.returncode, 0, model_doctor.stderr)
            self.assertIn("AEGIS MODEL DOCTOR", model_doctor.stdout)
            model_doctor_json = run_cli("--json", "models", "doctor", cwd=tmp)
            self.assertEqual(model_doctor_json.returncode, 0, model_doctor_json.stderr)
            self.assertTrue(next(check for check in json.loads(model_doctor_json.stdout)["checks"] if check["name"] == "metadata_only")["ok"])
    ...
    def test_subagents_delegate_runs_bounded_workers(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["generated_artifact_count"], 5)
            self.assertRegex(payload["root"]["artifacts"][0]["id"], r"^artifact-coordinator-")
            self.assertEqual(payload["root"]["artifacts"][0]["kind"], "final_synthesis")
            self.assertRegex(payload["root"]["final_synthesis"]["id"], r"^synthesis-")
            self.assertFalse(payload["root"]["final_synthesis"]["external_model_invocation_performed"])
            ...
            artifacts = run_cli("subagents", "--artifacts", cwd=tmp)
            self.assertEqual(artifacts.returncode, 0, artifacts.stderr)
            self.assertIn("SUBAGENT ARTIFACTS", artifacts.stdout)
            self.assertIn(planner_artifact, artifacts.stdout)
            artifact_json = run_cli("--json", "subagents", "--artifact", planner_artifact, cwd=tmp)
            self.assertEqual(artifact_json.returncode, 0, artifact_json.stderr)
            artifact_payload = json.loads(artifact_json.stdout)
            self.assertEqual(artifact_payload["artifact"]["id"], planner_artifact)
            self.assertIn("# Planner Artifact", artifact_payload["artifact"]["content"])
            artifact_search = run_cli("subagents", "--artifact-search", "Checkpoint plan", cwd=tmp)
            self.assertEqual(artifact_search.returncode, 0, artifact_search.stderr)
            self.assertIn("SUBAGENT ARTIFACT SEARCH", artifact_search.stdout)
            self.assertIn(planner_artifact, artifact_search.stdout)
            synthesis = run_cli("subagents", "--synthesis", payload["root"]["id"], cwd=tmp)
            self.assertEqual(synthesis.returncode, 0, synthesis.stderr)
            self.assertIn("SUBAGENT SYNTHESIS", synthesis.stdout)
            self.assertIn("artifact-coordinator", synthesis.stdout)
            graph = run_cli("subagents", "--artifact-graph", payload["root"]["id"], cwd=tmp)
            self.assertEqual(graph.returncode, 0, graph.stderr)
            self.assertIn("SUBAGENT ARTIFACT GRAPH", graph.stdout)
            self.assertIn(planner_artifact, graph.stdout)
            graph_json = run_cli("--json", "subagents", "--artifact-graph", payload["root"]["id"], cwd=tmp)
            self.assertEqual(graph_json.returncode, 0, graph_json.stderr)
            self.assertEqual(len(json.loads(graph_json.stdout)["graph"]["nodes"]), 4)
            self.assertEqual(len(json.loads(graph_json.stdout)["graph"]["edges"]), 5)
    ...
    def test_subagents_cli_reuses_artifact_only_after_approval_and_audits_safety_flags(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("subagent.coordinator.synthesis.completed", audit_text)
            self.assertIn('"external_model_invocation_performed": false', audit_text)
    ...
    def test_agents_delegate_routes_workers_to_ready_provider_without_browser(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                raw_secret = "sk-" + ("b" * 24)
                result = run_cli("agents", "delegate", f"improve terminal orchestration {raw_secret}", cwd=tmp, extra_env={"AEGIS_TEST_OPENAI_KEY": "test-key"})
                ...
                self.assertNotIn(raw_secret, sent)
                ...
                self.assertEqual(payload["artifact_count"], 5)
                self.assertEqual(len(seen["bodies"]), 4)
                planner_artifact = next(worker["artifacts"][0]["id"] for worker in payload["workers"] if worker["role"] == "planner")
                ...
                self.assertIn("subagent.coordinator.synthesis.completed", audit)
                self.assertIn('"external_model_invocation_performed": true', audit)
                self.assertIn('"browser_auto_launch": false', audit)
    ...
    def test_agents_surface_wraps_bounded_subagent_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual([worker["role"] for worker in payload["workers"]], ["planner", "researcher", "implementer", "reviewer"])
            planner_artifact = next(worker["artifacts"][0]["id"] for worker in payload["workers"] if worker["role"] == "planner")
            artifacts = run_cli("agents", "artifacts", cwd=tmp)
            self.assertEqual(artifacts.returncode, 0, artifacts.stderr)
            self.assertIn("AGENT ARTIFACTS", artifacts.stdout)
            self.assertIn(planner_artifact, artifacts.stdout)
            artifact = run_cli("agents", "artifacts", "show", planner_artifact, cwd=tmp)
            self.assertEqual(artifact.returncode, 0, artifact.stderr)
            self.assertIn("AGENT ARTIFACT", artifact.stdout)
            self.assertIn("# Planner Artifact", artifact.stdout)
            search = run_cli("agents", "artifacts", "search", "Checkpoint plan", cwd=tmp)
            self.assertEqual(search.returncode, 0, search.stderr)
            self.assertIn("AGENT ARTIFACT SEARCH", search.stdout)
            self.assertIn(planner_artifact, search.stdout)
            ...
            self.assertTrue(any(row["id"] == planner_artifact for row in json.loads(search_json.stdout)["artifacts"]))
            synthesis = run_cli("agents", "synthesis", payload["root"]["id"], cwd=tmp)
            self.assertEqual(synthesis.returncode, 0, synthesis.stderr)
            self.assertIn("AGENT SYNTHESIS", synthesis.stdout)
            graph = run_cli("agents", "graph", payload["root"]["id"], cwd=tmp)
            self.assertEqual(graph.returncode, 0, graph.stderr)
            self.assertIn("AGENT ARTIFACT GRAPH", graph.stdout)
            graph_json = run_cli("--json", "agents", "graph", payload["root"]["id"], cwd=tmp)
            self.assertEqual(graph_json.returncode, 0, graph_json.stderr)
            self.assertEqual(len(json.loads(graph_json.stdout)["graph"]["nodes"]), 4)
